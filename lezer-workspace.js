import {JavascriptVisitors} from "/home/kdog3682/2024-javascript/staging/JavascriptVisitors.js"
import {NameCheckerVisitors} from "/home/kdog3682/2024-javascript/staging/NameCheckerVisitors.js"
import {visit} from "/home/kdog3682/2024-javascript/staging/Visitor.js"
import * as python from "/home/kdog3682/2024-javascript/staging/python.js"
import * as javascript from "/home/kdog3682/2024-javascript/staging/javascript.js"
...
/* deno-fmt-ignore */ import {traverseBare, simpleTraverse, traverse, collectImports, is, evalTraversal, traverseWithEnvironmentAndRef, traverseJavascript, simpleCodeLibrary, iterateNode, findChildNodeRecursively, Environment, traverseJson, findallChildNodes, jsonTraversal, stringBuilder, isContentNode, iterateTree, getChildNodes, findChildNode, viewTree, traverseBlue, getNameNode, getStatef} from "/home/kdog3682/2024-javascript/stdlib/lezer/functions.js"
/* deno-fmt-ignore */ import {createState} from "/home/kdog3682/2024-javascript/stdlib/lezer/createState.js"

const s = `
async function* foo(bar = 2, sadf = {a:1}) {
    bar = 1
    const a = 1
}
------------------------------------------------------------
...
console.abc.log(aaa, ggg, aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa)
------------------------------------------------------------
var asdf = /regex/gi
------------------------------------------------------------
...
const a = abc.map(({a = c,b}) => x + 1)

------------------------------------------------------------

var dfg = async function   () {}
------------------------------------------------------------
const a =    class asdf {}
------------------------------------------------------------
...
foo()
import {importa, importb} from "/home/kdog3682/2024-javascript/staging/JavascriptVisitors.js"
goo(importb)
class Foo {
    constructor(options = boo) {
...
import {NameCheckerVisitors} from "/home/kdog3682/2024-javascript/staging/NameCheckerVisitors.js"
class Fo {
    constructor(options = Foo) {
        
    }
}
// Fo()
    // const a = self.visit(ast.children[0][1][2], env)         // the name
// \`\${a}\`
------------------------------------------------------------
`

// const file = '/home/kdog3682/2024-javascript/staging/abc.js'
// s = read(file)
const t = getLast(dashSplit(s))
const state = await createState(t)
// const node = state.node
// const ast = traverseJavascript(state)
// console.loggg(clip(traverseBare(state)))
// console.loggg(traverseJavascript2(state))
const ast = traverseJavascript2(state)
// exit(ast)
// clip(ast)
const visitorOpts = {
    singleQuotes: true,
...
// const options = {entry: 'name', visitors: NameCheckerVisitors, options: visitorOpts, state: {imports: []}}
const options = {entry: 'name', visitors: JavascriptVisitors, options: visitorOpts}
const envOpts = {relaxed: true, fallback: true}
const scope = {}
const args = [new Environment(null, scope, envOpts)]
console.log(visit(ast, options, ...args))


function jspy(filetype, ref) {
    const top = {
        javascript,
        python,
    }
    const master = top[filetype]
    const walker = (v, k) => {
        const value = master[k]
        return isDefined(value) ? value : v
    }
    return dict(ref, walker)
}
function get(filetype) {
    return jspy(filetype, {
        ignorableLeafNodes: [],
        attrLeafNodes: {}
    })
}
function traverseJavascript2(state) {
    const { ignorableLeafNodes, attrLeafNodes } = get(state.filetype)

    function onLeaf(node, value) {
        const name = node.name
        if (ignorableLeafNodes.includes(value)) {
            return
        } else if (name in attrLeafNodes) {
            return { type: "attr", value: attrLeafNodes[name]}
        } else {
            if (name == value) {
                // saver(name)
            }
    ...
    function onBranch(node, args, depth) {
        ...
        const payload = { name, children }
        if (exists(attrs)) payload.attrs = attrs
        // if (exists(children)) payload.children = children
        return payload
    }
    return simpleTraverse(state, { onLeaf, onBranch })
}
// 
