...
const state = await createState(getLast(dashSplit(s)))
...
const options = {entry: 'name', visitors: JavascriptVisitors}
const args = [new Environment(null, {relaxed: true})]
console.loggg(visit(ast, options, ...args))

function visitChildren(self, node, ...args) {
    const children = []
    let child = node.firstChild
    if (child) {
        do {
            push2(self.visit(child, ...args))
        } while ((child = child.nextSibling))
        return children
    }
}

// const result = visit(node, {visitors, entry: 'type'})
// console.log(result)

// const result = visit(ast, {visitors, entry: 'type'})
// console.log(result)
