...
const NameCheckerVisitors = {
    ...
    ImportDeclaration(ast, env) {
        ...
        if (a.name == "ImportGroup") {
            const imports = this.visit(a, env)
    ...
    ImportGroup(ast, env) {
        ...
        this.visitAll(imports, env)
        return imports.map((imp) => imp.value)
    },
    default(ast, env) {
        this.visitAll(ast, env)
    },
...
function checker(ast, oldEnv) {
    const newEnv = oldEnv.createEnv()
    if (ast.children.length == 3) {
        const [a, b, c] = ast.children
        if (a.name == 'MethodDeclaration') {
            const B = this.visit(b, newEnv, oldEnv)
            const C = this.visit(c, newEnv)
        } else {
            const A = this.visit(a, oldEnv)
            const B = this.visit(b, newEnv, oldEnv)
            const C = this.visit(c, newEnv)
        }
    } else {
        const [b, c] = ast.children
        const B = this.visit(b, newEnv, oldEnv)
        const C = this.visit(c, newEnv)
    }
}

function checkEnv(self, ast, rootEnv) {
    const walker = (env) => {
        for (const item of env.missing) {
            if (rootEnv.variables.has(item)) {
                rootEnv.touch(item)
            } else {
                missing.add(item)
            }
