...
class Vim {
    ...
    cmd(s) {
        console.log('vim cmd:', s)
    }
}
class Api {
    expand() {
        return '/home/kdog3682/2023/vim.js'
    }
    nvim_buf_get_name(key) {
        
    }
    nvim_get_buf_by_handle(num) {
        return '/home/kdog3682/2024/1.js'
    }
}
...
function getBuffer(buffer) {
    if (isNumber(buffer)) {
        return vim.api.nvim_get_buf_by_handle(buffer)
    }
    if (buffer) {
        return buffer
    }
...
function openBuffer(buffer, mode = 'default') {
    const buf = getBuffer(buffer)
    const computed = pathTransformers[mode](buf)
    vim.cmd(`edit ${computed}`)
}
...
function jspy(key, filetype) {
    ...
    return buildjspy()[filetype][key]
}

function buildjspy() {
    const text = read('/home/kdog3682/2024/jspy.txt')
    const [raw, fm] = parseFrontmatter(text)
    const order = fm.order
    const items = split(raw, /^(\S+):/m)
    if (items[0] == '') {
        items.shift()
    }
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        if (isEven(i)) {
            ...
            for (let i = 0; i < order.length; i++) {
                ...
                deepAssign(store, lang, name, value)
            }
        }
    }
    return store
...
const pathTransformers = {
    default(path) {
        if (isAbsolutePath(path) && isFile(path)) {
            return path
        }
        const filetype = getFiletype(path)
        const defaultDirectory = jspy('defaultDirectory', filetype)
        if (defaultDirectory) {
            return npath(defaultDirectory, path)
        }
    }
}
