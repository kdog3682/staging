export { addMissingDependencyVisitors, AMDVisitor }
...
/* @bookmark 1711724426  */
const addMissingDependencyVisitors = {
    wrapper,
    transform,
    Script: { default: true },
    WhileStatement: { default: true },
    Block: { default: true },
    ArgList: { default: true },
    ParamList: { default: true },
    ArrowFunction: { default: true },
    CallExpression: { 
        default: true
    },

    ForStatement(ast, env) {
        return visitChildren(this, ast, env)
    },
    ForSpec(ast, env) {
    ...
    ForOfSpec(ast, env) {
        ...
        const prefix = ast.children[0].attrs?.constPrefix
            ? "const"
            : "let"
        return { prefix, identifier: a, value: b }
    },
    ...
    ParenthesizedExpression(ast, env) {
        return {
            ...
            valueLiteral: visitFirst(this, ast, env),
        }
    },
...
class ImportDeclaration {
    constructor(self, ast, env) {
        this.lang = self.state.filetype
        this.visit = self.visit // already bound
        // this.file =
        // this.imports =
        // this.importType =
        // this is returning an unhydrated item
    }
    ...
    lua() {
        const pack = removeExtension(tail(this.file))
        const moduleName = "module"
        const top = `local ${moduleName} = require("${pack}")`
        const mapper = (imp) => {
            return `local ${imp} = ${moduleName}.${imp}`
        }
        const imports = this.imports.map(mapper)
        return join(top, imports)
    }
    toJSON() {
    }
}

class CommentRefactor {
    /* refactor: reorganize */
    constructor() {
    }
}
...
function wrapper(value, ast, env) {
    const name = ast.name
    if (isArray(value)) {
        return grouping(name, value)
    }
    if (isObject(value)) {
    }
    return value
...
function smartString(x) {
    if (x instanceof Grouping) {
        return String(x)
    }
    if (x.valueLiteral) {
...
function appendChild(self, child) {
    ...
    try {
        if (isString(child)) {
            console.log('skipping child string:', child)
            // pause(child)
            return 
        }
        child.parent = self
...
class Grouping {
    constructor(children, parent) {
        ...
        this.children = []
        this.state = {}
        children.map((child) => appendChild(this, child))
        this.init && this.init()
        this.run && this.run()
    }
    toString() {
        const kids = this.children.map(smartString)
        if (this.template) {
...
function block(children, o = {}) {
    let s = children.map(smartString)
    s = join3(s)
    if (o.indent) {
        s = indent2(s)
    }
...
class Block extends Grouping {
    ...
    getItems() {
        const children = this.children.map(smartString)
...
function grouping(name, value, parent) {
    const ref = {
        Script,
        Block,
        CallExpression,
    ...
    return new ref[name](value, parent)
}

function transform(ref) {
    return function visitor(ast, env) {
        const name = ast.name
        if (ref.default) {
            return this.visitAll(ast, env)
        }
        if (ref.abc) {
            const [a, b] = this.visitAll(ast, env)
            // const [a, b] = visitFirstLeaveSecond(this, ast, env)
            pause(b)
        }
    }
}
function visitFirstLeaveSecond(self, ast, env) {
    const a = ast.children[0]
    const first = self.visit(a, env)
